...
class Application(tk.Frame):
    def __init__(self, master=None):
        ...
        self.item_colors = ['IndianRed1', 'SteelBlue1', 'SpringGreen1', 'Goldenrod1', 'LightPink1', 'LightSteelBlue1',
                            'DarkSeaGreen1', 'Khaki1']
    ...
    def layout_control_panel(self):
        ...
        for y in range(1, self.item_count + 1):
            ...
            cb1 = ttk.Combobox(self.control_panel, textvariable=self.sides[y], state="readonly",
                               values=['top', 'bottom', 'left', 'right'], width=7)
            cb1.bind("<<ComboboxSelected>>", self.layout_items)
            cb1.grid(column=y, row=2)

            self.fills.append(tk.StringVar())
            self.fills[y].set("none")
            cb2 = ttk.Combobox(self.control_panel, textvariable=self.fills[y], state="readonly",
                               values=['none', 'x', 'y', 'both'], width=7)
